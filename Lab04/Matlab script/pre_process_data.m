...
Z=h(:,ktrain);[NR,NC]=size(Z);Z=reshape(Z,NR*NC,1);[~,C]=kmeans(Z,Kquant,'MaxIter',200);C=sort(C);%C stores the centroids
...
% ----------------PD patients
% find the centroids for the training signals
Z=p(:,ktrain);[NR,NC]=size(Z);Z=reshape(Z,NR*NC,1);[~,C]=kmeans(Z,Kquant,'MaxIter',200);C=sort(C);%C stores the centroids
% perform quantization on all the signals
pq=cell(1,K);
for k=1:K
